// src/pages/Questions.jsx (for example)
...
// 🔁 Adjust these paths to your actual file structure
import useAuth from "../../contexts/useAuth";
import { TestContext } from "../../contexts/TestContext";
import FormulaFormatter from "../../contexts/FormulaFormatter";
import { API_BASE_URL } from "../../utils/config"; // or wherever you keep this

export default function QuestionsPage() {
  const [questions, setQuestions] = useState([]); // holds only current page
  const [loading, setLoading] = useState(false);
  const [topics, setTopics] = useState([]);
  const [portions, setPortions] = useState([]); // kept because you store it in filters
  ...
  const [searchParams] = useSearchParams(); // ✅ React Router v6 signature
  ...
  const resetPagination = useCallback(() => {
    setCurrentPage(1);
    navigate(`/admin/questions?page=1`);
  }, [navigate]);
  ...
  useEffect(() => {
    if (testData?.filters && subjects.length > 0 && questionTypes.length > 0) {
      ...
      if (
        ...
        subjects.some(
          (s) => s.value.toString() === filters.selectedSubject.toString()
        )
      ) {
      ...
      if (
        filters.selectedQuestionType &&
        questionTypes.some(
          (qt) => qt.value.toString() === filters.selectedQuestionType.toString()
        )
      ) {
  ...
  }, [testData, subjects, questionTypes]);

  // Initialize chapter filter after chapters are loaded
  useEffect(() => {
    if (testData?.filters && chapters.length > 0) {
      const filters = testData.filters;
      if (
        filters.selectedChapter &&
        chapters.some(
          (c) => c.value.toString() === filters.selectedChapter.toString()
        )
      ) {
  ...
  // Initialize topic filter after topics are loaded
  useEffect(() => {
    if (testData?.filters && topics.length > 0) {
      ...
      if (
        ...
        topics.some(
          (t) => t.value.toString() === filters.selectedTopic.toString()
        )
      ) {
    ...
    const fetchFilters = async () => {
      try {
        ...
        const [portionRes, subjectRes, questionTypeRes] = await Promise.all([
          axios.get(`${API_BASE_URL}/portions`, {
        ...
        setPortions(
          portionRes.data.map((p) => ({ value: p.id, label: p.name }))
        );
        setSubjects(
        ...
        setQuestionTypes(
          questionTypeRes.data.map((qt) => ({ value: qt.id, label: qt.name }))
        );
      } catch (error) {
    ...
    debounce(async (subjectId) => {
      try {
        ...
        const response = await axios.get(
          `${API_BASE_URL}/chapters/chapter/${subjectId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      try {
        ...
        const response = await axios.get(
          `${API_BASE_URL}/topics/topic/${selectedChapter}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  const fetchQuestions = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/questions/paginated`,
        {
          headers: { Authorization: `Bearer ${token}` },
          params,
        }
  ...
  const paginate = useCallback(
    (pageNumber) => {
      ...
      navigate(`/admin/questions?page=${pageNumber}`);
    },
    [navigate]
  );
      ...
      <div className="p-4 md:px-4 max-w-7xl mx-auto">
        ...
        <div className="mb-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Select
            ...
            value={
              subjects.find(
                (s) => s.value.toString() === selectedSubject?.toString()
              ) || null
            }
            onChange={(option) => {
          ...
          <Select
            ...
            value={
              chapters.find(
                (c) => c.value.toString() === selectedChapter?.toString()
              ) || null
            }
            onChange={(option) => {
          ...
          <Select
            ...
            value={
              topics.find(
                (t) => t.value.toString() === selectedTopic?.toString()
              ) || null
            }
            onChange={(option) => {
          ...
          <Select
            ...
            value={
              questionTypes.find(
                (qt) =>
                  qt.value.toString() === selectedQuestionType?.toString()
              ) || null
            }
            onChange={(option) => {
                      ...
                      <li className="border rounded-lg shadow-sm transition-shadow">
                        ...
                        >
                          <div className="flex items-start space-x-4">
                            <span className="text-gray-600 font-bold pt-1">
                              {serialNumber}.
                            </span>
                            <div>
